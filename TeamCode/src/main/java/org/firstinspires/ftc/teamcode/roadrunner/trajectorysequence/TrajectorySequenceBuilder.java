...
public class TrajectorySequenceBuilder {
    ...
    private TrajectorySequenceBuilder addPath(AddPathCallback callback) {
        ...
        double displacementDifference = builtTraj.getPath()
                                                 .length() - lastDisplacementTraj;
        ...
        for (SequenceSegment segment : sequenceSegments) {
            if (segment instanceof TrajectorySegment) {
                ...
                double displacement = thisSegment.getTrajectory().getPath()
                                                 .project(point, 0.25);
                Vector2d projectedPoint = thisSegment.getTrajectory().getPath()
                                                     .get(displacement).vec();
