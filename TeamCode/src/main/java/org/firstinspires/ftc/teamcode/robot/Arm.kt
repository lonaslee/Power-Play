...
class Arm(hardwareMap: HardwareMap, private val telemetry: Telemetry) {
    ...
    private val control = PIDController(0.009, 0.0, 0.0007)
    private val downControl = PIDFController(0.003, 0.0, 0.0006, 0.0)
    ...
    fun update() {
        downControl.setPIDF(dP, 0.0, dD, 0.0)
        val curPos = low.currentPosition - 170.0

        val pow = if (goingDown && height == GROUND) {
            downControl.calculate(
                curPos, GROUND.pos.toDouble()
            ) + dCos * cos(Math.toRadians(height.pos / TICKS_IN_DEGREES))
        } else {
            control.calculate(
    ...
    enum class Height(val pos: Int) {
        GROUND(-170), LOW(70), MID(170), BACKMID(320), BACKLOW(390), STACK(-120);
